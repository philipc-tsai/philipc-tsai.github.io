var gTicketList = 'LOVEMORE/TicketList';
var gTickets = 'LOVEMORE/Tickets';
var gTicketIndex = 'LOVEMORE/TicketsIndex';
var gTicketRowsCount = 0;
var gIsADMIN = false;
var gQROOT = 'THREE';
var gQDATA = 'THREE/QDATA';
var gQTime = 'THREE/QTIME';
            ...
            for (var i = 1; i < aTickets.length; i++) {
                try {
                    ...
                    if (aCont.length > 3) {
                        var oTicket = new TicketClass();
                        //oTicket.AssignedId = cleanString(aCont[1]);
                        oTicket.AssignedTo = cleanString(aCont[1]);
                        oTicket.Chapter = cleanString(aCont[2]);
...
function downloadTicketsDetailed(divName, sortName) {
    ...
    tickets.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
            ...
            sHTML = "<tr>";
            sHTML += "<td>" + oTicket.TicketNo + "</td><td>" + oTicket.AssignedTo + "</td>";
        ...
        sHTML = '<tr><td colspan="7">TOTAL : ' + gTicketRowsCount.toString() + '</td></tr>';
        $('#' + divName).append(sHTML);
    }, function (error) {
        alert("Error: " + error.code);
    });
...
function downloadTicketsUnsold(divName, sortName) {
    var nCols = 3;
    var sHTML = '';
    var sAssigned = '';
    var nTicketNo = 0;
    ...
    tickets.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
            ...
            if (!oTicket.Sold || oTicket.Sold != 'YES') {
                sHTML = "<tr>";
                sHTML += "<td>" + oTicket.TicketNo + "</td><td>" + oTicket.AssignedTo + "</td>";
                sHTML += "<td>" + (oTicket.Comment ? oTicket.Comment : "") + "</td>";
                sHTML += "</tr>";
        ...
        sHTML = '<tr><td colspan="3">TOTAL : ' + gTicketRowsCount.toString() + '</td></tr>';
        $('#' + divName).append(sHTML);
    }, function (error) {
        alert("Error: " + error.code);
    });
...
function downloadTicketsSoldPaid(divName, sortName) {
    var nCols = 3;
    var sHTML = '';
    var sAssigned = '';
    var nTicketNo = 0;
    ...
    tickets.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
            ...
            if (oTicket.Sold == 'YES' && oTicket.Paid == 'YES') {
                sHTML = "<tr>";
                sHTML += "<td>" + oTicket.TicketNo + "</td><td>" + oTicket.AssignedTo + "</td>";
                sHTML += "<td>" + (oTicket.Sold ? oTicket.Sold : "NO") + "</td><td>" + (oTicket.SoldTo ? oTicket.SoldTo : "") + "</td>";
                sHTML += "<td>" + (oTicket.Paid ? oTicket.Paid : "NO") + "</td><td>" + (oTicket.Accounted ? oTicket.Accounted : "NO") + "</td>";
...
function downloadTicketsSoldUnPaid(divName, sortName) {
    var nCols = 3;
    var sHTML = '';
    var sAssigned = '';
    var nTicketNo = 0;
    ...
    tickets.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
            ...
            if (oTicket.Sold == 'YES' && oTicket.Paid != 'YES') {
                ...
                sHTML += "<td>" + oTicket.TicketNo + "</td><td>" + oTicket.AssignedTo + "</td>";
                sHTML += "<td>" + (oTicket.Sold ? oTicket.Sold : "NO") + "</td><td>" + (oTicket.SoldTo ? oTicket.SoldTo : "") + "</td>";
                sHTML += "<td>" + (oTicket.Paid ? oTicket.Paid : "NO") + "</td>";
                sHTML += "<td>" + (oTicket.Comment ? oTicket.Comment : "") + "</td>";
                sHTML += "</tr>";
                $('#' + divName).append(sHTML);
                gTicketRowsCount++;
        ...
        sHTML = '<tr><td colspan="6">TOTAL : ' + gTicketRowsCount.toString() + '</td></tr>';
        $('#' + divName).append(sHTML);
    }, function (error) {
        alert("Error: " + error.code);
    });
...
function downloadTicketsPaidNotAccounted(divName, sortName) {
    var nCols = 3;
    var sHTML = '';
    var sAssigned = '';
    var nTicketNo = 0;
    ...
    tickets.once("value", function (snapshot) {
        snapshot.forEach(function (childSnapshot) {
            ...
            if (oTicket.Sold == 'YES' && oTicket.Paid == 'YES' && oTicket.Accounted != 'YES') {
                sHTML = "<tr>";
                sHTML += "<td>" + oTicket.TicketNo + "</td><td>" + oTicket.AssignedTo + "</td>";
                sHTML += "<td>" + (oTicket.Sold ? oTicket.Sold : "NO") + "</td><td>" + (oTicket.SoldTo ? oTicket.SoldTo : "") + "</td>";
                sHTML += "<td>" + (oTicket.Paid ? oTicket.Paid : "NO") + "</td><td>" + (oTicket.Accounted ? oTicket.Accounted : "NO") + "</td>";
        ...
        snapshot.forEach(function (childSnapshot) {
            ...
            if (bValid) {
                ...
                if (sAssignedTo != oTicket.AssignedTo) {
                    ...
                    sHTML = '<tr>';
                    if (bIsAdmin)
                        sHTML += '<td class="LeftText"><a style="font-weight:bold;text-decoration:underline;"  href="lovemore.html?seller=' + sAssignedTo + '&admin=YES" target="_blank">' + sAssignedTo + '</a></td>';
                    else
                        sHTML += '<td class="LeftText"><a style="font-weight:bold;text-decoration:underline;"  href="lovemore.html?seller=' + sAssignedTo + '" target="_blank">' + sAssignedTo + '</a></td>';
                    ...
                    if (cDanceMove != '') {
                        sHTML += '<td><img src="images/lovemore/dance0' + cDanceMove + '.gif" style="width:40px;height:30px" /></td>';
                    } else {
                        sHTML += '<td></td>';
                    }
...
function sold_update(nTicketNo) {
    var sTicketNo = zeroFill(nTicketNo, 3);
...
function setQData(sData) {
    firebase.database().ref(gQDATA).set(sData);
}
function setQDataTime(sData) {
    firebase.database().ref(gQTime).set(sData);
}
function getQData(sDivName) {
    try {
        var oData = firebase.database().ref(gQDATA);
        oData.on("value", function (snapshot) {
            $('#' + sDivName).prop('innerText', snapshot.val())
        });
    } catch (e) {
        alert(e.Description);
    }
}
function getQDataTime(sDivName) {
    try {
        var oData = firebase.database().ref(gQTime);
        oData.on("value", function (snapshot) {
            $('#' + sDivName).prop('innerText', snapshot.val())
        });
    } catch (e) {
